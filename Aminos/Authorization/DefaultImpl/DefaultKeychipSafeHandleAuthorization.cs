...
{
    private static readonly ConcurrentDictionary<string, AuthorizationCached> cached = new();
    private static readonly ConcurrentBag<string> bad = new();

    private readonly AminosDB aminosDB;
    private readonly ILogger<DefaultKeychipSafeHandleAuthorization> logger;

    public DefaultKeychipSafeHandleAuthorization(AminosDB aminosDB,
        ILogger<DefaultKeychipSafeHandleAuthorization> logger)
    {
    ...
    private string ConvertSerialToSafeHandle(string keychip)
    {
        return "sh_" + SimpleCryptography.Encrypt(keychip).Replace("=", "_");
    }
