...
public class VerifyKeychipAttribute : ActionFilterAttribute
{
    ...
    public override async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        ...
        bool result;
        if (!context.RouteData.Values.TryGetValue("safeHandle", out var strObj) || strObj is not string safeHandle ||
            string.IsNullOrWhiteSpace(safeHandle))
        {
            result = false;
        }
        else
        ...
        else
        {
            ...
            await context.HttpContext.Response.Body.WriteAsync(
                Encoding.UTF8.GetBytes("Invalid safeHandle, you have to poweron on your machine."));
        }
    }
}