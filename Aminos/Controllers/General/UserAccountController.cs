...
public class UserAccountController : CommonWebAPIControllerBase
{
    ...
    [HttpPost("Register")]
    public async ValueTask<IActionResult> Register([FromForm] string userName, [FromForm] string passwordHash,
    ...
    [HttpPost("Update")]
    public async ValueTask<IActionResult> Update([FromForm] string newPasswordHash, [FromForm] string newEmail,
    ...
    [HttpGet("GetGameplayLogs")]
    public async ValueTask<IActionResult> GetGameplayLogs([FromQuery] DateTime fromDateTime,
        [FromQuery] DateTime toDateTime)
    {
        var user = await GetCurrentRequestUser();

        var result = await handler.GetGameplayLogs(user, fromDateTime, toDateTime);
        return Json(result);
    }

    [Authorize(AuthRolePolicyString.AdminRole)]
    [HttpPost("ChangeUserRole")]
    public async ValueTask<IActionResult> ChangeUserRole([FromForm] Guid targetUserGuid,
        [FromForm] AuthRolePolicy newRole)
    {
        var user = await GetCurrentRequestUser();
