...
using Aminos.Services.UserActivityLogger;
...
public class MaimaiDXUserAllHandler
{
    ...
    private readonly IUserActivityLogger userLogger;

    public MaimaiDXUserAllHandler(ILogger<MaimaiDXUserAllHandler> logger, IUserActivityLogger userLogger,
        MaimaiDXDB maimaiDxDB, AminosDB aminosDB)
    {
        this.logger = logger;
    ...
    public async ValueTask<UpsertResponseVO> UpsertUserAll(UserAllRequestVO request)
    {
        ...
        if (await maimaiDxDB.UserDetails.FirstOrDefaultAsync(x =>
                x.Id == request.userId) is UserDetail storedUserDetail)
        {
        ...
        else
        {
            ...
            var user = await aminosDB.UserAccounts.FirstOrDefaultAsync(x =>
                x.Cards.Any(y => y.Luid == accessCode));
            if (user is not null)
            {
                //log user activity
                userLogger.LogActivity(user, ActivityType.GamePlay, isFirstPlay ? "第一次游玩maimai DX" : "游玩maimai DX");
                userLogger.LogGameplay(user, "sdez");
            }
        }
                ...
                foreach (var upsertDetail in user2pPlaylog.User2pPlaylogDetails)
                {
                    ...
                    if (await maimaiDxDB.User2pPlaylogDetails.FirstOrDefaultAsync(x =>
                            ((x.UserDetail2 == upsertDetail.UserDetail2 && x.UserDetail1 == upsertDetail.UserDetail1)
                             || (x.UserDetail1 == upsertDetail.UserDetail2 &&
                                 x.UserDetail2 == upsertDetail.UserDetail1))
    ...
    private ValueTask SimpleRepalceEntites<T>(IEnumerable<T> inList, ICollection<T> existList,
        Func<T, T, bool> childCmp) where T : class
    {
        if (inList != null)
