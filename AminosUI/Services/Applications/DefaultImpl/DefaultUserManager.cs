...
internal class DefaultUserManager : IUserManager
{
    ...
    public async ValueTask<CommonApiResponse> LoginByCookies(CookieContainer container, CancellationToken cancellation)
    {
        applicationHttpFactory.Cookies = container;
        var resp = await applicationHttpFactory.GetAsCommonApi<UserAccount>("api/Account/Get", default, cancellation);
        if (resp.isSuccess)
            CurrentUser = resp.obj;
        return resp;
    }
